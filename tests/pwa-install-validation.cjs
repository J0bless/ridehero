...
assert.match(html, /js\/navigation\.js\?v=13/, 'the navigation script must use the current cache-busting version');
assert.match(worker, /\.\/css\/onboarding\.css\?v=9/, 'the service worker must precache the current onboarding stylesheet');
assert.match(worker, /\.\/js\/navigation\.js\?v=13/, 'the service worker must precache the current navigation script');
...
assert.match(html, /css\/smart-entry\.css\?v=4/, 'the Smart Entry plan preview stylesheet must use a fresh cache key');
