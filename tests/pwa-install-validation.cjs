...
assert.match(worker, /const CACHE_NAME = 'ridehero-shell-v10'/);
assert.match(html, /css\/onboarding\.css\?v=4/, 'the onboarding stylesheet must use the current cache-busting version');
assert.match(html, /js\/navigation\.js\?v=2/, 'the navigation script must use the current cache-busting version');
assert.match(worker, /\.\/css\/onboarding\.css\?v=4/, 'the service worker must precache the current onboarding stylesheet');
