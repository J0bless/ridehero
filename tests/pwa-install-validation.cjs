...
assert.match(html, /js\/auth-client\.js\?v=6/, 'the OAuth callback client must use the current cache-busting version');
assert.match(worker, /\.\/js\/auth-client\.js\?v=6/, 'the service worker must precache the current OAuth callback client');
assert.match(html, /js\/auth-ui\.js\?v=5/, 'the OAuth callback UI must use the current cache-busting version');
assert.match(worker, /\.\/js\/auth-ui\.js\?v=5/, 'the service worker must precache the current OAuth callback UI');
assert.match(html, /css\/onboarding\.css\?v=9/, 'the onboarding stylesheet must use the current cache-busting version');
assert.match(html, /js\/navigation\.js\?v=17/, 'the navigation script must use the current cache-busting version');
assert.match(worker, /\.\/css\/onboarding\.css\?v=9/, 'the service worker must precache the current onboarding stylesheet');
assert.match(worker, /\.\/js\/navigation\.js\?v=17/, 'the service worker must precache the current navigation script');
assert.match(html, /css\/auth\.css\?v=5/, 'the Account email-code treatment must use a fresh cache key');
assert.match(worker, /\.\/css\/auth\.css\?v=5/);
