...
const wordmark = fs.readFileSync(path.join(root, 'icons', 'ridehero-wordmark.png'));
...
assert.match(worker, /const CACHE_NAME = 'ridehero-shell-v17'/);
assert.match(html, /css\/onboarding\.css\?v=8/, 'the onboarding stylesheet must use the current cache-busting version');
assert.match(html, /js\/navigation\.js\?v=9/, 'the navigation script must use the current cache-busting version');
assert.match(worker, /\.\/css\/onboarding\.css\?v=8/, 'the service worker must precache the current onboarding stylesheet');
assert.match(worker, /\.\/js\/navigation\.js\?v=9/, 'the service worker must precache the current navigation script');
assert.match(worker, /\.\/css\/auth\.css\?v=1/);
...
assert.match(worker, /\.\/js\/supabase-config\.js\?v=1/);
assert.match(worker, /\.\/js\/auth-client\.js\?v=1/);
assert.match(worker, /\.\/js\/auth-ui\.js\?v=1/);
assert.match(worker, /\.\/js\/account-friends\.js\?v=1/);
assert.match(worker, /\.\/js\/friends-ui\.js\?v=2/);
assert.match(worker, /\.\/js\/park-map\.js\?v=1/);
assert.match(worker, /\.\/icons\/ridehero-wordmark\.png/);
assert.match(worker, /\.\/js\/route-session\.js\?v=1/, 'the lightweight route session must be available offline');
assert.match(worker, /\.\/js\/growth-loader\.js\?v=2/, 'the lightweight growth loader must be available offline');
assert.match(html, /css\/growth-engine\.css\?v=2/, 'the updated Growth Engine styles must use a fresh cache key');
assert.match(html, /js\/growth-loader\.js\?v=2/, 'the updated Growth Engine loader must use a fresh cache key');
