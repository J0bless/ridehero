...
assert.match(html, /js\/auth-client\.js\?v=2/, 'the OAuth callback client must use the current cache-busting version');
assert.match(worker, /\.\/js\/auth-client\.js\?v=2/, 'the service worker must precache the current OAuth callback client');
assert.match(html, /js\/auth-ui\.js\?v=3/, 'the OAuth callback UI must use the current cache-busting version');
assert.match(worker, /\.\/js\/auth-ui\.js\?v=3/, 'the service worker must precache the current OAuth callback UI');
assert.match(html, /css\/onboarding\.css\?v=9/, 'the onboarding stylesheet must use the current cache-busting version');
assert.match(html, /js\/navigation\.js\?v=14/, 'the navigation script must use the current cache-busting version');
assert.match(worker, /\.\/css\/onboarding\.css\?v=9/, 'the service worker must precache the current onboarding stylesheet');
assert.match(worker, /\.\/js\/navigation\.js\?v=14/, 'the service worker must precache the current navigation script');
assert.match(html, /css\/auth\.css\?v=3/, 'the Account focus treatment must use a fresh cache key');
assert.match(worker, /\.\/css\/auth\.css\?v=3/);
assert.match(worker, /\.\/css\/friends\.css\?v=2/);
assert.match(worker, /\.\/css\/park-map\.css\?v=2/);
assert.match(html, /css\/smart-entry\.css\?v=5/, 'the Smart Entry queue preview stylesheet must use a fresh cache key');
...
assert.match(worker, /\.\/js\/supabase-config\.js\?v=3/);
...
assert.match(worker, /\.\/js\/smart-entry\.js\?v=1/);
assert.match(worker, /\.\/js\/ride-intelligence\.js\?v=1/);
assert.match(worker, /\.\/data\/park-catalog\.js\?v=2/, 'the sourced park geofences must use the current cache key');
assert.match(worker, /\.\/js\/wait-provider\.js\?v=2/, 'the operating-set wait semantics must use the current cache key');
assert.match(worker, /\.\/js\/growth-analytics\.js\?v=2/);
assert.match(worker, /\.\/icons\/ridehero-wordmark\.png/);
assert.match(worker, /\.\/js\/location-service\.js\?v=2/, 'the centralized location service must use the current cache key');
assert.match(worker, /\.\/js\/storage-migration\.js\?v=2/, 'the updated state schema must use the current cache key');
assert.match(worker, /\.\/js\/route-session\.js\?v=2/, 'the lightweight route session must be available offline');
assert.match(html, /js\/growth-loader\.js\?v=3/, 'the corrected sharing loader must use a fresh cache key');
assert.match(worker, /\.\/js\/growth-loader\.js\?v=3/, 'the lightweight growth loader must be available offline');
assert.match(html, /css\/growth-engine\.css\?v=2/, 'the updated Growth Engine styles must use a fresh cache key');
assert.match(html, /js\/growth-loader\.js\?v=3/, 'the updated Growth Engine loader must use a fresh cache key');
