...
assert.match(worker, /const CACHE_NAME = 'ridehero-shell-v16'/);
assert.match(html, /css\/onboarding\.css\?v=8/, 'the onboarding stylesheet must use the current cache-busting version');
assert.match(html, /js\/navigation\.js\?v=8/, 'the navigation script must use the current cache-busting version');
assert.match(worker, /\.\/css\/onboarding\.css\?v=8/, 'the service worker must precache the current onboarding stylesheet');
assert.match(worker, /\.\/js\/navigation\.js\?v=8/, 'the service worker must precache the current navigation script');
assert.match(worker, /\.\/css\/friends\.css\?v=1/);
assert.match(worker, /\.\/css\/park-map\.css\?v=1/);
assert.match(worker, /\.\/js\/friends-store\.js\?v=1/);
assert.match(worker, /\.\/js\/friends-ui\.js\?v=1/);
