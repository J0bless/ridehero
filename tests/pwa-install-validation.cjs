...
assert.match(worker, /const CACHE_NAME = 'ridehero-shell-v13'/);
assert.match(html, /css\/onboarding\.css\?v=7/, 'the onboarding stylesheet must use the current cache-busting version');
assert.match(html, /js\/navigation\.js\?v=5/, 'the navigation script must use the current cache-busting version');
assert.match(worker, /\.\/css\/onboarding\.css\?v=7/, 'the service worker must precache the current onboarding stylesheet');
assert.match(worker, /\.\/js\/navigation\.js\?v=5/, 'the service worker must precache the current navigation script');
