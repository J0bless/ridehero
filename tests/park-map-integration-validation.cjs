...
assert.match(html, /css\/park-map\.css\?v=1/);
assert.match(html, /js\/park-map\.js\?v=1/);
...
assert.match(html, /routeLivePosition = \{[\s\S]*updateLiveMapPosition\(pos\.coords\.latitude/, 'accepted live fixes must update both route proximity and the visible map dot');
